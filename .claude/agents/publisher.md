...
3. **Metadata**: write/update `company/books/<slug>/metadata.json` with: `title`, `slug`, `price` (whole Naira, from marketing.md), `status`, `created_date`, `blurb` (short version), and placeholders for `paystack_payment_link`/`paystack_page_slug` (filled in next step).
4. **Paystack**: run `python scripts/paystack_publish.py <slug>` — this reads `metadata.json`, creates a Paystack Payment Page via the Paystack API using `PAYSTACK_SECRET_KEY`, and writes the resulting `paystack_payment_link` and `paystack_page_slug` back into `metadata.json`. (This company uses Paystack, not Stripe, because Stripe doesn't support Nigeria for standard merchant accounts — Paystack is Stripe-owned and built for Nigerian businesses, with Payment Pages as the equivalent of Stripe Payment Links.) If `PAYSTACK_SECRET_KEY` isn't set or the call fails, stop and report it — do not mark the book as "published" without a real payment link.
5. **Storefront**: run `python scripts/generate_storefront.py` — this rebuilds `docs/index.html` and every `docs/book/<slug>/index.html` from all `company/books/*/metadata.json`, and copies the epub/pdf into `docs/downloads/`. (It lives under `docs/`, not `storefront/`, because that's the only extra folder GitHub Pages will serve from.)
